...
# open stdout line buffered to automatically flush prints
sys.stdout = os.fdopen(sys.stdout.fileno(), 'w', 1)

FILENAMES = {'writeup3': ["assignment3.pdf"]}
...
def main(args):
    ...
    extensions = set(map(lambda x: os.path.splitext(x)[-1][1:].lower(), image_files))
    if not (4 <= len(image_files) <= 8):
        raise RuntimeError("Wrong number of images found in submission folder. " +
                           "You must submit at least 4 images and no more than 8.")
    elif 0 < len(extensions) < 1:
        raise RuntimeError("Wrong number of image types found in submission folder " +
                           "All images must be of only one type. Choose from: {!s}".format(IMG_EXTS))
    args.filenames.extend(image_files)

    final_files = glob.glob('final.*')
    extensions = set(map(lambda x: os.path.splitext(x)[-1][1:].lower(), final_files))
    if len(extensions) != 1:
        raise RuntimeError("Wrong number of final images found in submission folder. " +
...
if __name__ == '__main__':
    ...
    setattr(args, 'filenames', FILENAMES[args.quiz])
